...
int main(int argc, char **argv) {
  ...
  MemArea untrusted_rom(top_scope + (".u_untrusted_device.u_instr_mem."
                                 "u_prim_rom.gen_generic.u_impl_generic"),
              0x4000 / 4, 4);

  MemArea untrusted_ram(top_scope + (".u_untrusted_device.u_ram_mem.u_mem."
                                 "gen_generic.u_impl_generic"),
              0x20000 / 4, 4);
  ...
  memutil.RegisterMemoryArea("untrusted_rom", 0x0000, &untrusted_rom);
  memutil.RegisterMemoryArea("untrusted_ram", 0x50000000u, &untrusted_ram);
