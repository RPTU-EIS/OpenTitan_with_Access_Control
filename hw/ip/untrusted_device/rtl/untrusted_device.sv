...
/**
 * Ibex RISC-V core with a slave TLUL interface to receive orders from a master core
 *
 * Instructions fetched from internal ROM
 *
...
module untrusted_device #(
  ...
  //Interrupt for receiving messages from master core
  logic        irq_fast_14;
...
// Route the Ibex instruction bus to the private ROM

  prim_rom_adv #(
    .Width(32),
    .Depth(2048),
    .MemInitFile(RomCtrlBootRomInitFile)
  ) u_instr_mem (
    .clk_i,
    .rst_ni,
    .req_i    (instr_req),
  ...
  tlul_adapter_host #(
    ...
    .wdata_i    (data_wdata),
    .be_i       (data_be),
    .valid_o    (m_data_rvalid),
    .rdata_o    (m_data_rdata),
    .err_o      (m_data_err),
    .intg_err_o (),
    .tl_o       (tl_d_ibex2fifo),
    .tl_i       (tl_d_fifo2ibex)
  ...
  tlul_adapter_slave #(
    ...
    .wdata_i (data_wdata),
    .be_i    (data_be),
    .valid_o (s_data_rvalid),
    .rdata_o (s_data_rdata),
    .err_o   (s_data_err),
    .irq_o   (irq_fast_14),
    .tl_o    (tl_s_ibex2fifo),
    .tl_i    (tl_s_fifo2ibex)
    ...
    .tl_h_i      (tl_s_i),
    .tl_h_o      (tl_s_o),
    .tl_s_o      (tl_s_fifo2ibex),
    .tl_s_i      (tl_s_ibex2fifo),
    .spare_req_i (1'b0),
    .spare_req_o (),
    .spare_rsp_i (1'b0),
    .spare_rsp_o ()
...
// Multiplex Ibex data bus to either TL master interface or TL slave interface
always_comb begin
  if (s_data_rvalid) begin
    data_rvalid   = s_data_rvalid;
    data_rdata    = s_data_rdata;
    data_err      = s_data_err;
  end else begin
    data_rvalid   = m_data_rvalid;
    data_rdata    = m_data_rdata;
    data_err      = m_data_err;
  end
  if (data_addr == SLAVE_ADDRESS || data_addr == HOST_ADDRESS) begin
    s_data_req    = data_req;
    m_data_req    = 0;
    data_gnt      = s_data_gnt;
  end else begin
